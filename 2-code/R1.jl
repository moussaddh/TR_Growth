...
df_R1 = DataFrame(Dates=selected, Growth_cm=growth_setp, cumul_cm=cumul_growth_setp)
...
let
    df_R1.Growth_mm = cm_to_mm.(df_R1[!, :Growth_cm])
    df_R1.cumul_mm = cm_to_mm.(df_R1[!, :cumul_cm])
end

XLSX.writetable("arbre_R1.xlsx", df_R1)
